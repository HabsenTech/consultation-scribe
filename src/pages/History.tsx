...
import { Loader2, Search, Calendar, User, FileText, Eye, Download, Filter, ChevronLeft, ChevronRight, History as HistoryIcon } from 'lucide-react';
...
const History = () => {
  ...
  const fetchConsultations = async () => {
    ...
    setIsLoading(true);
    try {
      const { consultations: data, total } = await api.getConsultations({
        page: currentPage,
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-foreground font-heading">Consultation History</h1>
              <p className="text-muted-foreground">View and manage your past consultations</p>
            </div>
          </div>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <Input placeholder="Search by patient name or diagnosis..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }} className="pl-10" />
              </div>
              <div className="flex gap-2">
                <div className="relative">
                  <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                  <Input type="date" value={dateFilter} onChange={(e) => { setDateFilter(e.target.value); setCurrentPage(1); }} className="pl-10 w-40" />
                </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm text-muted-foreground">Showing {consultations.length} of {totalCount} consultations</p>
        </div>
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-muted-foreground">
                {searchQuery || dateFilter ? 'Try adjusting your search or filter criteria' : 'Start recording consultations to see them here'}
              </p>
            </CardContent>
          </Card>
        ) : (
                ...
                <CardContent className="py-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h3 className="font-semibold text-foreground">{consultation.patient_name}</h3>
                        <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground mt-1">
                          {consultation.patient_age && <span>{consultation.patient_age}</span>}
                          {consultation.patient_gender && (<><span>•</span><span className="capitalize">{consultation.patient_gender}</span></>)}
                          <span>•</span>
                          <span>{format(new Date(consultation.consultation_date), 'PPP')}</span>
                        </div>
                        {consultation.diagnosis && (
                          <Badge variant="secondary" className="mt-2">
                            {consultation.diagnosis.length > 50 ? `${consultation.diagnosis.substring(0, 50)}...` : consultation.diagnosis}
                          </Badge>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-2 sm:flex-shrink-0">
                      ...
                      <Button variant="outline" size="sm" onClick={() => handleDownloadPDF(consultation)}>
              ...
              <div className="space-y-6">
                ...
                {decodedReport.patientInfo?.symptoms && decodedReport.patientInfo.symptoms.length > 0 && (
                  <div>
                    ...
                    <ul className="list-disc list-inside text-sm text-muted-foreground">
                      {decodedReport.patientInfo.symptoms.map((symptom, idx) => <li key={idx}>{symptom}</li>)}
                    </ul>
                  </div>
                )}
                <div>
                  <h4 className="font-semibold text-foreground mb-2">Diagnosis</h4>
                  <p className="text-sm text-muted-foreground">{decodedReport.diagnosis || selectedConsultation.diagnosis || 'Not specified'}</p>
                </div>
                ...
                {decodedReport.advice && decodedReport.advice.length > 0 && (
                  <div>
                    ...
                    <ul className="list-disc list-inside text-sm text-muted-foreground">
                      {decodedReport.advice.map((a, idx) => <li key={idx}>{a}</li>)}
                    </ul>
                  </div>
                )}
