...
const Settings = () => {
  ...
  const [preferences, setPreferences] = useState({
    defaultLanguage: 'en',
    showAdvice: true, showInvestigations: true, showDietChart: true,
    showPastHistory: true, showDrugHistory: true, showFamilyHistory: true,
    showVaccinationHistory: true, showBirthHistory: true, showPregnancyHistory: true,
    defaultFollowUpDays: 7, defaultAdvice: '',
  });
  ...
  useEffect(() => {
    const fetchPreferences = async () => {
      if (!user) return;
      try {
        const { preferences: data } = await api.getPreferences();
        if (data) {
          setPreferences({
            defaultLanguage: 'en',
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg"><Globe className="w-5 h-5 text-primary" />Language Preferences</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="flex items-center justify-between">
                  <Label htmlFor="defaultLanguage">Default Recording Language</Label>
                  <Select value={preferences.defaultLanguage} onValueChange={(v) => setPreferences(p => ({ ...p, defaultLanguage: v }))}>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg"><FileText className="w-5 h-5 text-primary" />Prescription Sections</CardTitle>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={key} className="flex items-center justify-between">
                      ...
                      <Switch id={key} checked={preferences[key as keyof typeof preferences] as boolean} onCheckedChange={(v) => setPreferences(p => ({ ...p, [key]: v }))} />
                    </div>
                  ))}
                </div>
              </CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg"><Bell className="w-5 h-5 text-primary" />Default Values</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="followUp">Default Follow-up Days</Label>
                    <Input id="followUp" type="number" min={1} max={90} value={preferences.defaultFollowUpDays} onChange={(e) => setPreferences(p => ({ ...p, defaultFollowUpDays: parseInt(e.target.value) || 7 }))} className="mt-1.5 w-24" />
                  </div>
                  <div>
                    <Label htmlFor="defaultAdvice">Default Advice (one per line)</Label>
                    <Textarea id="defaultAdvice" placeholder="Rest adequately&#10;Drink plenty of fluids" value={preferences.defaultAdvice} onChange={(e) => setPreferences(p => ({ ...p, defaultAdvice: e.target.value }))} className="mt-1.5" rows={4} />
                  </div>
                </div>
              </CardContent>
            </Card>
